...
function Mode_card(){
    ...
    if(!n) this.error.push("Ϊ!");
    if(!tel) this.error.push("绰벻Ϊ!");
    //if(!this._isTeleNum(tel)) this.error.push("Чĵ绰룡");
    //    if(!org) this.error.push("λΪ!");
    //    if(!til) this.error.push("ְλΪ!");
    //    if(!url) this.error.push("ַΪ!");
    //    if(!this._isUrl(url)) this.error.push("Чַ");
    //    if(!mail) this.error.push("䲻Ϊ!");
    //    if(!this._isMail(mail)) this.error.push("Ч!");
    //    if(!adr) this.error.push("ַΪ!");
    this.string = "MECARD:";
...
function Mode_telephone(){
    ...
    if(!tel) this.error.push("绰벻Ϊ!");
    //if(!this._isTeleNum(tel)) this.error.push("Ч绰!");
    this.string = 'tel:'+tel;
}
/* sms class */
function Mode_sms(){
...
function onload(){
    ...
    addEvent("qrcodeform", "submit", function(event){
        ...
        if(object.getError().length){
            ...
            $('bigcode').innerHTML='<div class="note">无</div>';
        }else{
            if(object.getString()){
                ...
                $('bigcode').innerHTML='<a><img src="'+uri+'" id="qrcodeimg"/></a>'
            }else{
...
/*ֳ*/
function chageSize(obj,max,addobj){//ֳȣ󳤶ȣӵ
